...
function GetAllBlogs() {
  ...
  const fetchBlogs = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/v2/blog/blogs?page=${currentPage}`);
...
aspect-square w-8 flex justify-center items-center text-yellow-700" />
  ...
  <>
    ...
    <span>...<Link to={`/blog/${blog._id}`} className="underline text-black">{" "}Read more</Link></span>
  </>
) : (
  blog.description
)}
