...
const SearchBlog = async(e)=>{
  ...
  try {
    const res = await fetch(`http://localhost:8000/api/v2/blog/search/${searchTerm}`)
            ...
            <div className='bg-white relative min-h-[40px] top-[70px] left-0   rounded-lg'>
             {
              searchResult.map((blog)=>(
                ...
                <div className='hover:bg-gray-200 cursor-pointer p-1 w-full'>{blog.title}</div>
                <div><img className='h-8 w-8' src={blog.avatar.url} alt="" /></div>
                </div>
                  </Link>
                </div>
              ))
